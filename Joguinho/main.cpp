...
int main() {
    ...
    sf::Texture spiderman_tex { loadTexture("spiderman.png") };
    sf::Texture sandman_tex { loadTexture("sandman.png") };
    sf::Texture combat_tex { loadTexture("Combat_Background.jpg") };

    const int STEP {100};

    Entity spiderman(1, 1, STEP, spiderman_tex);
    Entity sandman(3, 3, STEP, sandman_tex);
    Board board(7, 5, STEP, combat_tex);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                ...
                moveEntity(event.key.code, spiderman, {sf::Keyboard::Left, sf::Keyboard::Up, sf::Keyboard::Right, sf::Keyboard::Down});
                moveEntity(event.key.code, sandman, {sf::Keyboard::A, sf::Keyboard::W, sf::Keyboard::D, sf::Keyboard::S});
            }
        }

        window.clear();
        board.draw(window);
        spiderman.draw(window);
        sandman.draw(window);
        window.display();
    }
